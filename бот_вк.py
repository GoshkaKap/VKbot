        ...
        if event.to_me:
            ...
            try: 
                if res['result']['final']['check']['check_result'] != [] :
                    ...
                    streets = sum(res['scores'][2:8] + res2['scores'][5:8])   / len(res['scores'][2:8] + res2['scores'][5:8])         
                    home    = sum(res['scores'][8:10] + res2['scores'][8:10]) / len([res['scores'][8:10]] + [res2['scores'][8:10]])\
                    
                    scores = [animals, ways, streets, home]
                    print(msg, max(scores) - min(scores))
                    if max(scores) - min(scores) <= 0.0 :
            ...
            send_some_msg(id, ['Мы уведомим Вас, когда Вы снова сможете писать сообщения'])
            time.sleep(20)
            send_some_msg(id, ['Теперь мы готовы ответить на Ваш вопрос'])
